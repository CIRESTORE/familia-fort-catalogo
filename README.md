...
Antes de abrir WhatsApp, el cliente diligencia nombre, celular, departamento, ciudad o municipio, dirección, barrio e indicaciones. Departamento y municipio usan desplegables con búsqueda por escritura sobre un listado local de Colombia para reducir errores de digitación. El mensaje usa texto compatible sin emojis, jerarquía y formato de WhatsApp e incluye los datos de entrega, productos, cantidades, referencias, precios, subtotales, total y condición de envío.

El carrito muestra una sola barra con las metas de `$80.000` (pedido mínimo) y `$100.000` (envío gratis). Usa estados de color diferentes antes del mínimo, con el pedido habilitado y con envío gratis; al superar `$100.000` aparece un distintivo visible `ENVÍO GRATIS`.
